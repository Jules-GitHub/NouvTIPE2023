...
def affichage(points, zone, e):
    ...
    plt.imshow(valeurs, cmap='viridis', extent=zone)
...
def affichage2(points, attracteurs, zone, e):
    ...
    for x in range(int(zone[0]/e), int(zone[1]/e)):
        ...
        for y in range(int(zone[2]/e), int(zone[3]/e)):
            c = couleur(valeurs[x][y])
            image[-1].append((c, 2.5*c*(1-c), 1-c, 0.9))
    ...
    def transp(tab):
        return np.array([[tab[i][j] for i in range(len(tab))] for j in range(len(tab[0]))])

    plt.imshow(transp(image), cmap='viridis', extent=zone)
...
avec_attracteur(2, 0, [0, 1, 0, 1], [], 0.01)